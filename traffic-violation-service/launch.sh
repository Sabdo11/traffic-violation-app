...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "traffic-violation-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    oc new-app traffic-violation-service:1.0-SNAPSHOT
    oc expose svc/traffic-violation-service
else

	echo "Launching the application locally..."
	pattern="traffic-violation-service"
	files=( $pattern )
	cd ${files[0]}

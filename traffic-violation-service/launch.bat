...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "traffic-violation-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name traffic-violation-service apps/traffic-violation-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app traffic-violation-service:1.0-SNAPSHOT
		call oc expose svc/traffic-violation-service
	) else (
